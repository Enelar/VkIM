...
void CSkinButton::SetIcon(int nIconInId, int nIconOutId)
{
...
BOOL CSkinButton::SetBtnCursor(int nCursorId)
{
...
void CSkinButton::SetFlat(BOOL bState)
{
  m_bIsFlat = bState;
  Invalidate();
} // End of SetFlat
...
BOOL CSkinButton::GetFlat()
{
  return m_bIsFlat;
} // End of GetFlat


void CSkinButton::SetAlign(int nAlign)
{
  switch (nAlign)
  {    
    case ST_ALIGN_HORIZ:
...
int CSkinButton::GetAlign()
{
  return m_nAlign;
} // End of GetAlign


void CSkinButton::DrawBorder(BOOL bEnable)
{
  m_bDrawBorder = bEnable;
} // End of DrawBorder


const char* CSkinButton::GetVersionC()
{
  return "2.5";
} // End of GetVersionC


const short CSkinButton::GetVersionI()
{
  return 25; // Divide by 10 to get actual version
} // End of GetVersionI


void CSkinButton::SetShowText(BOOL bShow)
{
  m_bShowText = bShow;
  Invalidate();
} // End of SetShowText
...
//pDC->FillRect(&itemRect, &br);
  ...
  if (bIsPressed)
  {
    if (m_bIsFlat == TRUE)
    {
     // if (m_bDrawBorder == TRUE)
      ...
      /*///}
		if(!gray){
		skin.btnDown.bmp.DrawStretched(pDC, itemRect.left, itemRect.top, itemRect.Width(), itemRect.Height(), 0, 0, 0, 0);
		}else{
			skin.btnGDown.bmp.DrawStretched(pDC, itemRect.left, itemRect.top, itemRect.Width(), itemRect.Height(), 0, 0, 0, 0);
		}
    }
    else    
    {
  ...
  else // ...else draw non pressed button
  {
    ...
    if (m_bIsFlat == TRUE)
    {
      if (m_MouseOnButton == TRUE && m_bDrawBorder == TRUE)
      {
        ...
        pDC->SelectObject(pOldPen);*/
		  if(!gray){
			skin.btnOver.bmp.DrawStretched(pDC, itemRect.left, itemRect.top, itemRect.Width(), itemRect.Height(), 0, 0, 0, 0);
		  }else{
			  skin.btnGOver.bmp.DrawStretched(pDC, itemRect.left, itemRect.top, itemRect.Width(), itemRect.Height(), 0, 0, 0, 0);
		  }
      }
    }
    else
  ...
  if(!m_MouseOnButton && !bIsPressed){
	  if(!gray){
		  skin.btnUp.bmp.DrawStretched(pDC, itemRect.left, itemRect.top, itemRect.Width(), itemRect.Height(), 0, 0, 0, 0);
	  }else{
		  skin.btnGUp.bmp.DrawStretched(pDC, itemRect.left, itemRect.top, itemRect.Width(), itemRect.Height(), 0, 0, 0, 0);
	  }
  }
  ...
  if (sTitle.IsEmpty() == FALSE)
  {
	  ...
	  pDC->SetTextColor(gray?skin.btnGTextSColor:skin.btnTextSColor);
	  itemRect.OffsetRect(0, 1);
	  pDC->DrawText(sTitle, itemRect, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
	  itemRect.OffsetRect(0, -1);
	  pDC->SetTextColor(gray?skin.btnGTextColor:skin.btnTextColor);
	  pDC->DrawText(sTitle, itemRect, DT_CENTER|DT_VCENTER|DT_SINGLELINE);
  }
...
void CSkinButton::DrawTheIcon(CDC* pDC, CString* title, RECT* rcItem, CRect* captionRect, BOOL IsPressed, BOOL IsDisabled)
{
...
void CSkinButton::SetDefaultInactiveBgColor(BOOL bRepaint)
{
...
void CSkinButton::SetInactiveBgColor(COLORREF crNew, BOOL bRepaint)
{
...
const COLORREF CSkinButton::GetInactiveBgColor()
{
	return m_crInactiveBg;
} // End of GetInactiveBgColor


void CSkinButton::SetDefaultInactiveFgColor(BOOL bRepaint)
{
...
void CSkinButton::SetInactiveFgColor(COLORREF crNew, BOOL bRepaint)
{
...
const COLORREF CSkinButton::GetInactiveFgColor()
{
	return m_crInactiveFg;
} // End of GetInactiveFgColor


void CSkinButton::SetDefaultActiveBgColor(BOOL bRepaint)
{
...
void CSkinButton::SetActiveBgColor(COLORREF crNew, BOOL bRepaint)
{
	m_crActiveBg = crNew; 
	if (bRepaint == TRUE) Invalidate();
...
const COLORREF CSkinButton::GetActiveBgColor()
{
	return m_crActiveBg;
} // End of GetActiveBgColor


void CSkinButton::SetDefaultActiveFgColor(BOOL bRepaint)
{
...
void CSkinButton::SetActiveFgColor(COLORREF crNew, BOOL bRepaint)
{
	m_crActiveFg = crNew; 
	if (bRepaint == TRUE) Invalidate();
...
const COLORREF CSkinButton::GetActiveFgColor()
{
	return m_crActiveFg;
} // End of GetActiveFgColor


void CSkinButton::SetFlatFocus(BOOL bDrawFlatFocus, BOOL bRepaint)
{
...
BOOL CSkinButton::GetDefault()
{
	return m_bIsDefault;
} // End of GetDefault


void CSkinButton::DrawTransparent()
{
...
void CSkinButton::PaintBk(CDC * pDC)
{
