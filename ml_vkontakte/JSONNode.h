...
#define foreach(iterator, container)\
    for (typeof(container.begin()) iterator = container.begin(), iterator##_end = container.end();\
         ...
         ++iterator)*/
//#define foreach(iterator, container) JSONNode** iterator;JSONNode* __it;for(int __i=0;__i<container.size(),__it=container[__i],iterator=&__it;__i++)

#define foreach(iterator, container)\
	JSONNode** iterator;JSONNode** iterator##_end;for(iterator = container.begin(), iterator##_end = container.end();\
         iterator != iterator##_end;\
         ++iterator)
