...
#endif
    json_char * result;
    json_char * runner = result = (json_char*)malloc((value_t.length() + 1) * sizeof(json_char));  //dealing with raw memory is faster than adding to a json_string
		  ...
		  case TEXT('\"'):  //a quote
			 *runner++ = TEXT('\"');
			 while(*(++p) != TEXT('\"')){  //find the end of the quotation, as white space is preserved within it
				libJSON_ASSERT_SAFE(*p, TEXT("Null terminator inside of a quotation"), goto endofloop;);
				switch(*p){
				    case TEXT('\\'):
					   *runner++ = TEXT('\\');
